...
public class SummaryWriter {
    ...
    public void writeAvg(PlannerSolution solution) {
        ...
        Map<Integer, Integer> shift_size = groupSize(solution);//calculates size of group for each shift

        nuberOfShiftsOfsize(shift_size);//how many shifts of a particular size there ware
    ...
    private Map<Integer, Integer> groupSize(PlannerSolution ps) {/// maps shift id to size
        ...
        for (Shift sh : shifts) {
            ...
            if (calc > 0) {
                colect.put(id, calc);// add only the shifts which are used
            }
        }
        return colect;
    }
