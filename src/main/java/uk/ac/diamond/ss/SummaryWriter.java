    ...
    private double percentage(PlannerSolution ps, int num) {
        ...
        for (Person pl: people){
            for (Allocation p : ps.getAllocations()) {
                if (p.getPerson().equals(pl) && pl.checkPreference(p.getShift()) == num){
                    calc++;
                }
            }
